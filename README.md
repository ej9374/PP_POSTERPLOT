...
- **회원가입 및 이메일 인증**
    - **Spring Security 및 JWT**를 활용한 회원가입 및 로그인 처리
    - Redis 기반 이메일 인증 코드 임시 저장 및 검증 처리
- **AI 모델 연동**
    - 유저가 업로드한 이미지를 **GCP storage**에 저장 후, 그 링크를 Flask AI모델 서버에 전달
    - **WebClient**를 통해 AI 모델 서버와 통신하여 줄거리 결과 수신
- **시나리오 및 피드백 시스템 구현**
    - 시나리오 게시글 CRUD 기능
    - 댓글 및 좋아요 기능을 통한 사용자 간 상호작용 구현
- **API 설계 및 문서화**
    - RESTFul API 설계 및 Swagger 문서 자동화 구성
- **서버 배포 및 운영**
    - Spring Boot 어플리케이션을 **GCP VM**에 배포
    - Docker를 이용한 백엔드 환경 컨테이너화
...
1. 회원가입 및 이메일 인증
    - 사용자는 아이디, 비밀번호, 이메일을 입력하여 회원가입 가능
    - 이메일 인증을 통해 본인 확인을 완료해야 회원가입 완료
2. 영화 포스터 업로드 및 줄거리 생성
    - 사용자가 2장의 영화 포스터를 업로드
    - 사전 학습된 BLIP 이미지 캡셔닝 모델과 GPT-Neo 자연어 생성 모델을 활용하여, 사용자가 업로드한 이미지로부터 영화 줄거리 초안을 자동 생성
3. 시나리오 등록
    - 사용자가 ai가 생성한 시나리오를 수정하여 시나리오 제목, 장르, 줄거리 등을 입력해 게시글 등록 가능
    - 시나리오는 DB에 저장되고 게시판에 게시
4. 시나리오 열람 및 피드백
    - 게시판에서 최신 시나리오 및 좋아요 순 인기 시나리오, 장르별 시나리오를 확인 가능
    - 시나리오 별로 좋아요, 댓글 기능을 통해 사용자 간 피드백 제공
